...
public abstract class General: System.Object
{
    ...
    public abstract void assignment_attempt(General target, General source);
}
...
public class Any : General
{
    ...
    public override void assignment_attempt(General target, General source)
    {
        if (source.GetType() == target.GetType()) target = source;
        else if ((source.GetType()).IsSubclassOf(target.GetType())) target = source;
        else target = new None();
    }
}
